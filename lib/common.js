...
let bl = {
	...
	"checkUserTenantAccess": (record, tenantObj, log, autoRoaming) => {
		if (record && record.tenant && tenantObj && tenantObj.id) {
			...
			if (autoRoaming && autoRoaming[tenantObj.id]) {
				if (autoRoaming[tenantObj.id][record.tenant.id]) {
					let response = {
						"groups": autoRoaming[tenantObj.id][record.tenant.id].groups || null,
						"tenant": {
							"id": tenantObj.id,
							"code": tenantObj.code
						}
