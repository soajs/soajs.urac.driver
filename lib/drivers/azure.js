    ...
    "init": function (req, cb) {
        ...
        let data = {
            ...
            configAuth: {
                ...
                identityMetadata: req.soajs.servicesConfig.passportLogin[mode].identityMetadata,
                ...
                validateIssuer: req.soajs.servicesConfig.passportLogin[mode].validateIssuer || true,
                ...
                issuer: req.soajs.servicesConfig.passportLogin[mode].issuer || null,
                ...
                passReqToCallback: req.soajs.servicesConfig.passportLogin[mode].passReqToCallback || true,

                // Optional, default value is clientID
                audience: req.soajs.servicesConfig.passportLogin[mode].audience || null,

                // Optional. Default value is false.
                // Set to true if you accept access_token whose `aud` claim contains multiple values.
                allowMultiAudiencesInToken: req.soajs.servicesConfig.passportLogin[mode].allowMultiAudiencesInToken || false,

                // Optional. 'error', 'warn' or 'info'
                loggingLevel: req.soajs.servicesConfig.passportLogin[mode].loggingLevel || 'info',

            }
        };

        // Required to be true to use B2C
        if (req.soajs.servicesConfig.passportLogin[mode].isB2C)
            data.configAuth.isB2C = req.soajs.servicesConfig.passportLogin[mode].isB2C;

        // Required to use B2C
        if (req.soajs.servicesConfig.passportLogin[mode].policyName)
            data.configAuth.policyName = req.soajs.servicesConfig.passportLogin[mode].policyName;

        // Optional.
        if (req.soajs.servicesConfig.passportLogin[mode].loggingNoPII)
            data.configAuth.loggingNoPII = req.soajs.servicesConfig.passportLogin[mode].loggingNoPII;

        // Optional.
        if (req.soajs.servicesConfig.passportLogin[mode].clockSkew)
            data.configAuth.clockSkew = req.soajs.servicesConfig.passportLogin[mode].clockSkew;

        // Optional.
        if (req.soajs.servicesConfig.passportLogin[mode].scope)
            data.configAuth.scope = req.soajs.servicesConfig.passportLogin[mode].scope;
