...
let bl = {
	...
	"save": (soajs, inputmaskData, modelObj, cb) => {
		...
		modelObj.getSocialNetworkUser(data, (err, record) => {
			...
			if (record) {
				...
				//TODO: add under record.config.social the following:
				// record.config.social: { "locked": ["firstName", "lastName", "email"]
				// this means do not modify these properties
				...
				if (allowedTenantObj) {
					...
					if (record.config && record.config.allowedTenants && Array.isArray(record.config.allowedTenants)) {
						for (let i = 0; i < record.config.allowedTenants.length; i++) {
							...
							if (temp_allowedTenantObj.tenant.id === allowedTenantObj.tenant.id) {
								inAllowed = true;
					...
					if (userRecord.groups) {
						if (!record.groups || !Array.isArray(record.groups) || record.groups.length === 0) {
							record.groups = userRecord.groups;
						}
					}
				}
	...
	"find": (soajs, inputmaskData, modelObj, cb) => {
		...
		let response_fn = (err, record) => {
			...
			if (record && record.status === "active") {
				...
				soajs.log.error("User [" + record.username + " - " + record.email + "] status is " + record.status);
		...
		} else if (inputmaskData.phone) {
			data.phone = inputmaskData.phone;
			modelObj.getUserByPhone(data, (err, record) => {
				response_fn(err, record);
			});
	...
	"assureConfig": (record, cb) => {
		...
		//Groups ACL
		if (record.groupsConfig && record.groupsConfig.length > 0) {
			let mergedInfo = { "allowedPackages": {} };
			async.each(record.groupsConfig, (g, callback) => {
				if (g.config && g.config.allowedPackages) {
					async.eachOf(g.config.allowedPackages, (arrayOfPackages, product, callback) => {
						if (!mergedInfo.allowedPackages[product]) {
							mergedInfo.allowedPackages[product] = arrayOfPackages;
						} else {
							mergedInfo.allowedPackages[product] = [...new Set([...mergedInfo.allowedPackages[product], ...arrayOfPackages])];
						}
						return callback();
					});
				}
